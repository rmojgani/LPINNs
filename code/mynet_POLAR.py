...
Lagrangian PINN on Polar Coordinate
"""
import torch
...
class LPINN_POLAR(nn.Module):
    # =========================================================================
    def __init__(self, N, M, HIDDEN, DEEPu, DEEPx, C, NU0, EQN_TYPE, IF_PLOT):
        # print('-----------------------------------')
        # print('Lagrangian PINN on Polar Coordinate')
        # print('-----------------------------------')
...
#            return x_t - c*u*(1-u)
    ...
    def forward(self, x, t, xx, tt):
        ...
        # xtheta = torch.cos(x*(2*pi)/2)
        # ytheta = torch.sin(x*(2*pi)/2)
        # for 0<x<2π
        xtheta = torch.cos(x)
        ytheta = torch.sin(x)
...
#        u =            (self.output(x5)).cuda()
        ...
        if self.DEEPu == 6:
            x4 = torch.tanh(self.hidden2(x3))
            x5 = torch.tanh(self.hidden2(x4))
            x6 = torch.tanh(self.hidden2(x5))
            u = (self.output(x6)).cuda()

        # xxtheta = torch.cos(xx*(2*pi)/2)
        # yytheta = torch.sin(xx*(2*pi)/2)
        
        # for 0<x<2π
        xxtheta = torch.cos(xx)
        yytheta = torch.sin(xx)
        
        xL0 = torch.cat((xxtheta, yytheta, tt), axis = 1)
...
#        xL3 = torch.tanh(self.hidden1(xL2))
    ...
    def loss_ru(self, x, t, xx, tt):
        """ ru : du/dt - F2() """
        rx_pred, ru_pred = self.net_residual(x, t, xx, tt)
    ...
    def loss_x0u0(self, x, t, xx, tt, u0, x0):
        ...
        # |Δx|-Δx
        # loss_dx = torch.diff(xL.reshape(M,N), axis=1)
        # loss_dx = torch.norm( torch.abs(loss_dx)-loss_dx )
        return loss_x0 + loss_u0 #+ 1*loss_dx
    # =========================================================================
...
#         return loss_ubp#loss_ub1 + loss_ub2
    ...
    def optimize_noclosure(self, x, t, 
        ...
        for epoch in tqdm(range(0, NUM_EPOCHS)):
            ...
            # loss_pupx = self.loss_pupx(x_batch.cuda(), t_batch.cuda(), 
                                       # x_batch.cuda(), t_batch.cuda() )
            # Total loss
            loss = GAMMA_RX*loss_rx + GAMMA_RU*loss_ru + GAMMA_IC*loss_x0u0
    ...
    def myplot(self, x, t, u0, x0, epoch):
        ...
        plt.savefig(train_path+'/LPINN'+str(epoch)+'.png')
    #%%
